...
⚠️ **Be careful with this setting** as it will delete all files which is not generated by webpack in your output directory defined by `.setOutputPath()`. So for example, if you've set `./` as your output path, it will wipeout your project. **Be sure to correctly define your output path before turning on output cleaning feature**.
...
You can add new plugins to the configuration by using `` method. You should pass an object containing options and an init callback just as below.
